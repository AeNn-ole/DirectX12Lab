...
{
    if (FAILED(hr)) {
        char buf[256]; std::snprintf(buf,sizeof(buf),"%s (hr=0x%08X)",what,(unsigned)hr);
...
void RenderingSystem::CollectVisibleInstances()
{
    ...
    {
        visible.resize(m_instances.size());
        for (int i = 0; i < (int)m_instances.size(); ++i) visible[i] = i;
    }
    else if (m_octreeCullingEnabled && m_octree.IsBuilt())
    {
        m_octree.Query(m_frustumPlanes, visible);
        visible.erase(
            std::remove_if(visible.begin(), visible.end(), [this](int i){
    ...
    const float distSq = m_billboardDistance * m_billboardDistance;

    for (int i : visible)
    {
        XMVECTOR center = XMLoadFloat3(&m_instances[i].Center);
        float d2 = XMVectorGetX(XMVector3LengthSq(XMVectorSubtract(center, eye)));
        if (d2 < distSq) m_nearIndices.push_back(i);
        else             m_farIndices.push_back(i);
    }

    m_lastVisibleCount = (int)(m_nearIndices.size() + m_farIndices.size());
}
...
{
    auto& L = m_lights;
    L[0].Type=0; L[0].Direction={0.4f,-0.8f,0.2f}; L[0].Color={1.f,0.95f,0.85f,0.7f};
...
void RenderingSystem::GeometryPass()
{
    ...
    for (uint32_t mi = 0; mi < N; ++mi) UpdateMaterialCB((int)mi);

    for (int i : m_nearIndices)
    {
        UpdateInstanceCB(i);
...
void RenderingSystem::LightingPass()
{
    ...
    depthToSRV.Transition = {
        m_depthStencilBuffer.Get(), D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES,
...
// ─────────────────────────────────────────────────────────────────────────────
void RenderingSystem::BillboardPass()
{
    const int numFar = (int)m_farIndices.size();
    if (numFar == 0) return;

    struct Center { float x, y, z, w; };
    Center* centers = reinterpret_cast<Center*>(m_mappedBillboardCenters);
    for (int i = 0; i < numFar; ++i) {
        const auto& c = m_instances[m_farIndices[i]].Center;
        centers[i] = { c.x, c.y, c.z, 0.f };
    }

    BillboardFrameCB fb{};
    XMMATRIX view = XMLoadFloat4x4(&m_view);
    XMMATRIX proj = XMLoadFloat4x4(&m_proj);
    XMStoreFloat4x4(&fb.ViewProj, XMMatrixTranspose(view * proj));

    XMFLOAT4X4 v; XMStoreFloat4x4(&v, view);
    fb.CamRight = { v._11, v._21, v._31 };
    fb.CamUp    = { v._12, v._22, v._32 };
    fb.Size     = { m_billboardSize, m_billboardSize };
    std::memcpy(m_mappedBillboardFrameCB, &fb, sizeof(fb));

    m_cmdList->SetPipelineState(m_billboardPSO.Get());
    m_cmdList->SetGraphicsRootSignature(m_billboardRootSig.Get());
    m_cmdList->RSSetViewports(1, &m_viewport);
    m_cmdList->RSSetScissorRects(1, &m_scissorRect);
    m_cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    ...
    ID3D12DescriptorHeap* heaps[] = { m_cbvSrvHeap.Get() };
    m_cmdList->SetDescriptorHeaps(1, heaps);
    m_cmdList->SetGraphicsRootConstantBufferView(0, m_billboardFrameCB->GetGPUVirtualAddress());

    const uint32_t N = m_numMaterials;
    D3D12_GPU_DESCRIPTOR_HANDLE texH = m_cbvSrvHeap->GetGPUDescriptorHandleForHeapStart();
    texH.ptr += (SIZE_T)((2*N + 4) * m_cbvSrvDescriptorSize);
    m_cmdList->SetGraphicsRootDescriptorTable(1, texH);

    D3D12_GPU_DESCRIPTOR_HANDLE cenH = m_cbvSrvHeap->GetGPUDescriptorHandleForHeapStart();
    cenH.ptr += (SIZE_T)((2*N + 5) * m_cbvSrvDescriptorSize);
    m_cmdList->SetGraphicsRootDescriptorTable(2, cenH);

    m_cmdList->DrawInstanced(6, (UINT)numFar, 0, 0);
}
...
void RenderingSystem::OnResize(uint32_t width, uint32_t height)
{
    ...
    XMStoreFloat4x4(&m_proj,
        XMMatrixPerspectiveFovLH(0.25f*XM_PI, (float)width/height, 0.1f, 5000.f));
}
    ...
    ComPtr<ID3D12Resource>& outTex)
{
    ...
    int w, h, ch;
    uint8_t* data = stbi_load(pathA, &w, &h, &ch, 4);
    if (data) {
        texW=(uint32_t)w; texH=(uint32_t)h;
        pixels.assign(data, data+texW*texH*4);
        stbi_image_free(data);
    } else {
        texW=texH=64; pixels.resize(texW*texH*4);
...

    {
        ...
        ThrowIfFailed2(upBuf->Map(0,&rr,(void**)&p),"Map Tex");
